...
def write_last_log_to_csv(source_log, output):
    with open(source_log) as f:
       ...
       header = base[0]
    result= {}
    sorted_data = sorted (base [1:], key = lambda x: convert_str_to_datetime(x[2]))
    for name, email, last_chang in sorted_data:
        result[email] = (name, email,last_chang)
...
if __name__=="__main__":
    write_last_log_to_csv("mail_log.csv","result_test4.csv")
